...
function filterSpeciesAbility(value, label){
    ...
    if(name){
        for(let i = 0; i < rows.length; i++){
            const key = rows[i].getElementsByClassName("key")[0].innerText
            if(!species[key]["abilities"].includes(name) && !species[key]["innates"].includes(name)){
                rows[i].classList.add(`hideFilter${label}${value}`.replaceAll(" ", ""))
            }
            rows[i].classList.remove("hide")
        }
...
function filterMovesFlag(value, label){
    const table = document.getElementsByClassName("activeTable")[0]
    const rows = table.querySelectorAll("tbody > tr")
    for(let i = 0; i < rows.length; i++){
        const key = rows[i].getElementsByClassName("key")[0].innerText
        rows[i].classList.add(`hideFilter${label}${value}`.replaceAll(" ", ""))
        for (let j = 0; j < moves[key]["flags"].length; j++){
            if(sanitizeString(moves[key]["flags"][j]) === value){
                rows[i].classList.remove(`hideFilter${label}${value}`.replaceAll(" ", ""))
            }
        }
        rows[i].classList.remove("hide")
    }
...
async function setFilters(){
    ...
    createFilterGroup(createFilterArrayFromObject(["flags"], moves), "Flag", [movesFilterList])
